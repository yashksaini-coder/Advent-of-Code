...
int main() {
    ...
    // Calculate the formula from the Rust code
    Dev dac = {'d', 'a', 'c'};
    Dev fft = {'f', 'f', 't'};
    Dev svr = {'s', 'v', 'r'};
    Dev out = {'o', 'u', 't'};
    ...
    size_t result = 
        paths(fwd, bwd, dac, fft) * paths(fwd, bwd, svr, dac) * paths(fwd, bwd, fft, out)
        + paths(fwd, bwd, fft, dac) * paths(fwd, bwd, svr, fft) * paths(fwd, bwd, dac, out);
